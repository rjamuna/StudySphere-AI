...
export default function Login() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)', padding: '1.5rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', top: '10%', left: '15%', width: 400, height: 400, background: 'radial-gradient(circle, rgba(91,95,239,0.08) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '10%', right: '15%', width: 300, height: 300, background: 'radial-gradient(circle, rgba(124,58,237,0.07) 0%, transparent 70%)', pointerEvents: 'none' }} />
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.4,0,0.2,1] }}
        ...
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: 'var(--text-2)', fontSize: '0.9rem' }}>Sign in to <span style={{ color: 'var(--primary)', fontWeight: 600 }}>StudySphere AI</span></p>
        </div>
        ...
        <div className="card" style={{ padding: '2rem', boxShadow: 'var(--shadow-lg)' }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.125rem' }}>
            ...
            <div className="input-group">
              ...
              <div style={{ position: 'relative' }}>
                <input id="password" className="input" type={showPw ? 'text' : 'password'} value={password}
                  ...
                  autoComplete="current-password" style={{ paddingRight: '2.75rem' }} />
            ...
            <motion.button className="btn btn-primary" type="submit" disabled={loading}
              ...
              {loading ? <><Spinner size={16} /> Signing in...</> : <>Sign In <ArrowRight size={16} /></>}
            </motion.button>
          </form>
        </div>
